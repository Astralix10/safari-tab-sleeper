...
on isSleepableURL(tabURL)
	set lowerURL to my lowerText(tabURL)
	if my isLocalSleeperURL(lowerURL) then return false
	if lowerURL starts with "http://" then return true
	if lowerURL starts with "https://" then return true
	return false
end isSleepableURL
		...
		repeat with wi from 1 to count of windows
			...
			repeat with ti from 1 to count of tabs of targetWindow
				if ti is not activeTabIndex then
					...
					if my isSleepableURL(originalURL) and (not my isAllowlistedURL(originalURL, allowlistPath)) then
						try
							set originalTitle to name of targetTab
						on error
							set originalTitle to originalURL
						end try
					set sleeperURL to (my sleepPageBaseURL(sleeperTarget)) & "#url=" & (my encodeQueryComponent(originalURL)) & "&title=" & (my encodeQueryComponent(originalTitle)) & "&reason=manual-all-except-current&auto=1"
